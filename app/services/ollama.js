...
export async function generateStoryFromImages ({ prompt, images, model = DEFAULT_MODEL }) {
  if (!Array.isArray(images) || images.length === 0) {
    const error = new Error('At least one image is required to generate a story.');
    error.status = 400;
    throw error;
  }
